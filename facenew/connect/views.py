        ...
        if request.method == 'POST':
            if UserCrontabSchedule.objects.filter(user_id=facebook.user.id).count():
                ...
                for cron in crontabs:
                    ...
                    periodic_task = PeriodicTask(name=task_name, task='facenew.tasks.tasks.publish', crontab=interval_crontab, enabled=True, args=[facebook.user.id, interval_crontab.id])
                    periodic_task.save()
